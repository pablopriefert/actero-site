...
<Steps>
  <Step title="Ouvre la fiche Actero sur le Shopify App Store">
    Depuis le [Shopify App Store](https://apps.shopify.com), cherche **Actero** et clique **Ajouter l'app**. Tu peux aussi partir d'Actero : **Intégrations → Shopify → Connecter** — on te redirige vers la fiche. Tu n'as aucune adresse à saisir : Shopify sait de quelle boutique il s'agit.
  </Step>
  ...
  <Step title="Approuve l'installation">
    Tu reviens automatiquement sur `actero.fr`, sur une page qui confirme que ta boutique est connectée. L'installation ne déclenche aucun paiement : tu choisis ta formule ensuite, et **elle est facturée par Shopify**, sur la même facture que le reste de tes apps.
  </Step>
...
## Surveillance de la bulle de chat

Une mise à jour de thème retire parfois l'app embed Actero de ta vitrine sans prévenir. L'agent cesse alors de recevoir les messages laissés sur ton site, sans qu'aucune erreur ne te le signale.

Actero vérifie **au moins toutes les 24 heures** que la bulle de chat s'affiche bien sur ta vitrine. Le contrôle charge ta page d'accueil dans un vrai navigateur et cherche la bulle rendue à l'écran — pas seulement la balise `<script>` dans le HTML. Il détecte donc aussi les cas où le script est bien présent mais où la bulle n'apparaît pas : fichier widget en 404, erreur JavaScript, ou politique de sécurité de contenu (CSP) du thème qui bloque le domaine Actero. Le contrôle est automatique dès que ton onboarding Shopify est terminé — rien à activer.

Tu reçois un email intitulé **« Votre agent ne reçoit plus de messages depuis votre site »** uniquement quand la bulle **disparaît** entre deux contrôles (le précédent la trouvait, celui-ci non). L'email indique l'URL contrôlée et la marche à suivre pour la réactiver depuis l'éditeur de thème Shopify.

<Note>
L'alerte ne se déclenche que sur une régression. Une boutique qui n'a jamais activé l'app embed ne reçoit pas d'email quotidien. Un site temporairement injoignable au moment du contrôle ne déclenche pas d'alerte non plus.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Le widget ne s'affiche pas sur ma boutique">
    ...
    Si la bulle disparaît après une mise à jour de thème, Actero t'en informe automatiquement par email — voir [Surveillance de la bulle de chat](#surveillance-de-la-bulle-de-chat).
  </Accordion>
