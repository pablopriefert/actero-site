...
<AccordionGroup>
  <Accordion title="L'agent ne trouve pas ma commande #1234">
    Vérifie que Shopify est bien en statut **Connecté** dans Intégrations. L'agent exige que le numéro de commande **et** l'email du demandeur correspondent à la même commande côté Shopify avant de renvoyer un résultat. Un visiteur anonyme du widget qui ne donne qu'un numéro obtient donc une demande d'email, pas la commande d'un autre client. Si le client a acheté en invité avec une autre adresse que celle utilisée dans la conversation, demande-lui de confirmer son email d'achat.
  </Accordion>
