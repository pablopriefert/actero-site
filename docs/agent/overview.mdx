...
Actero prend aussi en charge la **famille GPT-5** d'OpenAI comme moteur alternatif (par défaut `gpt-5.4-mini` : rapide, qualité comparable pour les réponses SAV courtes). Le provider et le modèle sont choisis côté plateforme — tu n'as rien à configurer, et le comportement de l'agent (prompts, garde-fous, format de sortie) reste identique quel que soit le modèle utilisé.
...
<AccordionGroup>
  ...
  <Accordion title="Que se passe-t-il si Claude est down ?">
    Actero bascule automatiquement sur le provider de secours (OpenAI si Claude est le provider actif, et inversement). Si les deux providers échouent, le run est marqué `error` et le ticket est automatiquement escaladé vers **À traiter** avec la raison `error`. Aucun client ne reçoit de réponse cassée.
  </Accordion>
</AccordionGroup>
