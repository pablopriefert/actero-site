...
description: "Comment l'agent IA SAV Actero traite un ticket Shopify de bout en bout : classification d'intention, contexte, génération, garde-fous et escalade."
---
...
Actero prend aussi en charge la **famille GPT-5** d'OpenAI comme moteur alternatif (par défaut `gpt-5.4-mini` : rapide, qualité comparable pour les réponses SAV courtes), ainsi qu'**OpenRouter** pour router vers d'autres modèles compatibles Chat Completions. Le provider et le modèle sont choisis côté plateforme — tu n'as rien à configurer, et le comportement de l'agent (prompts, garde-fous, format de sortie) reste identique quel que soit le modèle utilisé.

<Note>
Tu peux suivre la consommation token par token dans **Activité → Historique des runs**. Chaque ligne affiche le modèle utilisé et le coût exact en USD.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Que se passe-t-il si Claude est down ?">
    Actero bascule automatiquement sur un provider de secours (Anthropic ↔ OpenAI, ou OpenRouter → Anthropic selon la configuration active). Si le failover échoue aussi, le run est marqué `error` et le ticket est automatiquement escaladé vers **À traiter** avec la raison `error`. Aucun client ne reçoit de réponse cassée.
  </Accordion>
</AccordionGroup>
