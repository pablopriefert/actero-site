...
Ton agent Actero est un moteur de support client IA spécialisé e-commerce (Shopify et WooCommerce). Il lit chaque message entrant, comprend ce que ton client veut, va chercher le bon contexte (commande, suivi, FAQ), rédige une réponse dans **ton** ton de marque, vérifie qu'elle ne casse aucune de tes règles, puis l'envoie ou escalade vers ton équipe humaine.
...
<Steps>
  ...
  <Step title="Récupération de contexte e-commerce">
    Si la classification est `order`, l'agent va lire la commande dans ta boutique connectée : Shopify (statut, tracking, items) ou [WooCommerce](/integrations/woocommerce#recherche-de-commandes) (statut, items — WooCommerce core ne fournit pas de tracking). Si c'est `product`, il cherche dans ton catalogue.
  </Step>
...
<AccordionGroup>
  <Accordion title="L'agent peut-il agir tout seul (rembourser, modifier une commande) ?">
    Non, par défaut. Les scopes Shopify et WooCommerce d'Actero sont en **lecture seule**. L'agent peut lire les commandes, mais pas les modifier ni rembourser. Toute action transactionnelle est escaladée vers ton équipe.
  </Accordion>
