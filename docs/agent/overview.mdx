...
Actero prend aussi en charge **GPT-5.5** d'OpenAI comme moteur alternatif. Le provider est choisi côté plateforme — tu n'as rien à configurer, et le comportement de l'agent (prompts, garde-fous, format de sortie) reste identique quel que soit le modèle utilisé.
...
<AccordionGroup>
  ...
  <Accordion title="Que se passe-t-il si Claude est down ?">
    Actero bascule automatiquement sur le provider de secours (OpenAI GPT-5.5 si Claude est le provider actif, et inversement). Si les deux providers échouent, le run est marqué `error` et le ticket est automatiquement escaladé vers **À traiter** avec la raison `error`. Aucun client ne reçoit de réponse cassée.
  </Accordion>
</AccordionGroup>
