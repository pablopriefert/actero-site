...
<ResponseField name="trial_ends_at" type="string|null">ISO timestamp de fin de période d'essai. Reste renseigné après expiration (l'essai n'est accordé qu'une fois par compte). `null` si le tenant n'a jamais eu d'essai. Pour savoir si le trial est en cours, compare la date à maintenant.</ResponseField>
