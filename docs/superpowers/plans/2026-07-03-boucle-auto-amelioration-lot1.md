...
- **`ai_recommendations` has CHECK constraints** (discovered during the smoke test): `category` ∈ growth|efficiency|risk|automation, `status` ∈ active|dismissed|implemented, `priority_level` ∈ high|medium|low (NOT NULL), `impact_score` 0–100. KB-gap recos therefore use `category='efficiency'`, `status='active'`, and a derived `priority_level`; the loop identifies its own rows via `source_version='improvement-loop-v1'` (that's the widget filter + endpoint gate + miner dedup key, NOT the category). Implemented tasks 2–4 reflect this.
