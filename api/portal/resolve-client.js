...
export default async function handler(req, res) {
  ...
  const { data, error } = await supabase
    .from('clients')
    .select('id, slug, brand_name, portal_enabled, portal_custom_domain, portal_logo_url, portal_primary_color, portal_display_name')
  ...
  return res.status(200).json({
    ...
    branding: {
      ...
      displayName: data.portal_display_name || data.brand_name,
    },
  });
}
