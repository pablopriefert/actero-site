    ...
    <div className="bg-white rounded-2xl border border-[rgba(0,0,0,0.06)] shadow-[0_2px_24px_rgba(0,0,0,0.06)] p-6 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap pt-4 border-t border-[#E5E5E5]">
        <button onClick={() => act('/api/portal/request-refund', { orderName: order.name, reason: 'via_portal' })}
          className="rounded-full border border-[rgba(0,0,0,0.08)] bg-white hover:bg-cream text-[#1A1A1A] font-medium px-5 py-2.5 text-[13px] transition">Demander un remboursement</button>
        <button onClick={() => act('/api/portal/request-return', { orderName: order.name, reason: 'via_portal' })}
          className="rounded-full border border-[rgba(0,0,0,0.08)] bg-white hover:bg-cream text-[#1A1A1A] font-medium px-5 py-2.5 text-[13px] transition">Lancer un retour</button>
        <button onClick={invoice}
          className="rounded-full border border-[rgba(0,0,0,0.08)] bg-white hover:bg-cream text-[#1A1A1A] font-medium px-5 py-2.5 text-[13px] transition">Télécharger ma facture</button>
      </div>
      {msg && (
        <div className="bg-cream border border-[rgba(0,0,0,0.06)] text-[#3A3A3A] rounded-2xl px-4 py-3 text-sm">
          {msg}
        </div>
      )}
    </div>
