...
const PRIORITY_CONFIG = {
  ...
  low: { label: 'Basse', color: { dark: 'bg-gray-50 text-[#716D5C] border-gray-200', light: 'bg-surface text-[#716D5C] border-gray-200' } },
}
...
const NewTicketForm = ({ onSubmit, onCancel, theme, isSubmitting }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={`p-6 rounded-2xl border ${isLight ? 'bg-white border-gray-200' : 'bg-surface border-gray-200'}`}
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 ${
              isLight
                ? 'bg-surface border-gray-200 text-[#262626] placeholder-slate-400'
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                  priority === key
                    ...
                    : (isLight ? 'border-gray-200 text-[#716D5C] hover:bg-surface' : 'border-gray-200 text-[#716D5C] hover:bg-gray-50')
                }`}
              >
                {config.label}
              </button>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={onCancel}
            className={`px-4 py-2 rounded-xl text-sm font-bold ${isLight ? 'text-[#716D5C] hover:bg-surface' : 'text-[#716D5C] hover:bg-gray-50'}`}
          >
            Annuler
          </button>
          <button
  ...
  return (
    <motion.div
      ...
      className={`p-4 rounded-2xl border transition-colors ${
        isLight ? 'bg-white border-gray-200 hover:border-slate-300' : 'bg-surface border-gray-200 hover:border-gray-300'
      }`}
    >
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className={`text-center py-16 rounded-2xl border ${isLight ? 'bg-white border-gray-200' : 'bg-surface border-gray-200'}`}>
