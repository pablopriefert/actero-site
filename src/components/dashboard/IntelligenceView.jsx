  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      />
      <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[540px] bg-surface shadow-2xl flex flex-col transform transition-transform duration-300 translate-x-0 overflow-y-auto border-l border-gray-200">
        <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-surface/90 backdrop-blur z-10">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-px before:bg-gradient-to-b before:from-white/10 before:to-transparent">
              {planSteps.map((step, idx) => (
                <div key={idx} className="relative flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-surface border border-gray-200 flex items-center justify-center flex-shrink-0 z-10 text-[#716D5C] font-bold text-sm shadow-sm mt-1">
                    {idx + 1}
                  </div>
                  <div className="bg-surface border border-gray-100 p-4 rounded-2xl flex-1 shadow-sm mt-1">
        ...
        {!isStarted && (
          <div className="p-6 md:p-8 border-t border-gray-200 bg-white flex flex-col sm:flex-row gap-3 sticky bottom-0 z-10">
            ...
            <button
              disabled={loading}
              onClick={onClose}
              className="flex-1 sm:flex-none bg-surface text-[#716D5C] font-bold py-3.5 px-6 rounded-xl border border-gray-200 hover:text-[#262626] hover:bg-gray-50 transition-colors disabled:opacity-50"
            >
              Annuler
            </button>
          </div>
...
export const RecommendationCard = ({ reco, onAction, onOpenPlan, theme = "dark" }) => {
  ...
  return (
    <div className={`border rounded-3xl p-6 transition-all duration-300 ${isLight ? "bg-white border-gray-200 shadow-sm hover:shadow-md" : "bg-surface border-gray-200 hover:border-gray-300"}`}>
      ...
      <div className={`pt-6 border-t flex flex-wrap items-center gap-3 ${isLight ? "border-gray-100" : "border-gray-100"}`}>
        ...
        <button
          ...
          className={`flex-1 lg:flex-none text-sm font-bold border px-6 py-2.5 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 ${isLight ? "bg-white border-gray-200 text-slate-700 hover:bg-surface" : "bg-gray-50 border-gray-200 text-[#716D5C] hover:bg-gray-50"}`}
        >
...
export const IntelligenceView = ({ supabase, setActiveTab, theme = "dark" }) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className={`border rounded-3xl shadow-sm overflow-hidden transition-colors duration-300 ${isLight ? "bg-white border-gray-200" : "bg-surface border-gray-200"}`}>
        ...
        <div className={`px-6 py-4 flex flex-wrap gap-4 items-center border-b ${isLight ? "bg-surface border-gray-100" : "bg-surface border-gray-100"}`}>
          <div className="flex items-center gap-2">
      ...
      {error ? (
        ...
        <div className="space-y-6">
          {[...Array(3)].map((_, i) => (
            <div key={i} className={`border rounded-3xl p-6 shadow-sm animate-pulse h-48 flex flex-col gap-4 ${isLight ? "bg-white border-gray-200" : "bg-surface border-gray-200"}`}>
        ...
        <div className={`border rounded-3xl p-16 text-center shadow-sm flex flex-col items-center ${isLight ? "bg-white border-gray-200" : "bg-surface border-gray-200"}`}>
