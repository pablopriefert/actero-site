  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div>
            <h2 className="text-lg font-bold text-white">Notes de call — {client?.brand_name}</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              {isDeployed ? 'Deploye' : isComplete ? 'Complet — pret pour le deploiement' : 'Brouillon'}
            </p>
          </div>
          <div className="flex items-center gap-2">
            {saving && <Loader2 className="w-4 h-4 text-gray-500 animate-spin" />}
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <React.Fragment key={step.id}>
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-px ${isDone ? 'bg-emerald-500/30' : 'bg-white/5'}`} />
                  )}
                </React.Fragment>
              );
            })}
        ...
        <div className="p-6 border-t border-white/10 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
    ...
    <div className={className}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder-gray-600 outline-none focus:border-white/20 transition-colors"
        />
        {suffix && (
  ...
  return (
    <div>
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={rows}
        className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder-gray-600 outline-none focus:border-white/20 transition-colors resize-none"
      />
    </div>
  );
}
...
function StepBasics({ form, updateField }) {
  return (
    ...
    >
      ...
      {/* Vertical selection */}
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-2">Verticale</label>
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${selected ? `text-${opt.color}-400` : 'text-gray-500'}`} />
                <span className={`text-sm font-medium ${selected ? 'text-white' : 'text-gray-400'}`}>{opt.label}</span>
  ...
  if (form.vertical === 'ecommerce') {
    return (
      ...
      >
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1.5">Outil de ticketing</label>
          <select
            value={form.ticketing_tool || ''}
            onChange={e => updateField('ticketing_tool', e.target.value || null)}
            className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white outline-none focus:border-white/20 transition-colors"
          >
        ...
        <div className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/10 rounded-lg">
          <span className="text-sm text-gray-300">Chatbot souhaite ?</span>
          <button
            onClick={() => updateField('wants_chatbot', !form.wants_chatbot)}
            className={`relative w-10 h-5 rounded-full transition-colors ${form.wants_chatbot ? 'bg-emerald-500' : 'bg-white/10'}`}
          >
...
function StepEmail({ form, updateField, onAutoDetect, autoDetecting }) {
  return (
    ...
    >
      ...
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-2">Preference d'envoi</label>
      ...
      {form.email_sending_preference === 'smtp_client' && (
        <div className="space-y-3 p-4 bg-white/[0.02] border border-white/10 rounded-lg">
      ...
      <div className="pt-2">
        <div className="flex items-center justify-between mb-3">
          <label className="text-xs font-medium text-gray-400">Liens du site (pour le brand context)</label>
          <button
      ...
      {workflows.map(wf => {
        ...
        return (
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium text-white">{wf.label}</p>
