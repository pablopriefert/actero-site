        ...
        <div className="px-6 py-8 flex flex-col items-center">
          {loading && (
            <div className="flex flex-col items-center py-12">
              <Loader2 className="w-8 h-8 animate-spin text-[#0F5F35]" />
          ...
          {!loading && !error && (
            <>
              ...
              <div className="relative mb-6">
                {started && (
                  <>
                    <span className="absolute inset-0 rounded-full bg-[#0F5F35]/20 animate-ping" />
                    <span className="absolute inset-0 rounded-full bg-[#0F5F35]/10 animate-pulse" />
                  </>
                )}
                <div className={`relative w-24 h-24 rounded-full flex items-center justify-center ${
                  started ? 'bg-[#0F5F35]' : 'bg-[#0F5F35]/10'
                }`}>
                  {started ? (
                    <Mic className="w-10 h-10 text-white" />
                  ) : (
                    <Mic className="w-10 h-10 text-[#0F5F35]" />
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                {!started ? (
                  <button
                    onClick={handleStart}
                    disabled={!agentId}
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#0F5F35] hover:bg-[#0c4e2b] disabled:opacity-50 text-white text-[13px] font-semibold transition-colors"
                  >
